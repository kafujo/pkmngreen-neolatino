...
; The Game Boy's CPU is an 8-bit processor, which means its registers are designed to hold 8 bits of data. The registers are named a, b, c, d, e, h, and l. These can be used individually for 8-bit (1-byte) data, or in pairs for 16-bit (2-byte) data (af, bc, de, and hl). The c register is often used in conjunction with the b register as the bc pair for operations that require 16-bit data, but it can also be used on its own for 8-bit operations.
; ld = load. de = registers d & e. Load the address of the TrainerInfo_NameMoneyTimeText into DE register pair
	ld de, TrainerInfo_NameMoneyTimeText
; use the PlaceString code routine (defined elsewhere) to display the string (series of letters, numbers, words, etc) at the (TrainerInfo_NameMoneyTimeText) address in DE
	call PlaceString
; hlcoord = hl coordinates (x, y). A "macro" (custom-defined bit of code instructions to enable easy reuse). Sets the HL register pair ("cursor") to point to a specific coordinate on the screen, where the next string or character will be displayed.
	hlcoord 7, 2
; w = signals a text (not number) variable
	ld de, wPlayerName
	call PlaceString
	hlcoord 8, 4
	ld de, wPlayerMoney
; start code to remove pokemon money symbol from front of sum on trainer card, adding yen symbol to after the sum instead
; Load the format (constant $e3) for printing BCD numbers into C register. BCD = Binary-Coded Decimal: A method of encoding decimal numbers where each digit is represented by its own binary sequence.
;	ld c, $e3 ; deleting this line entirely causes numbers to run on after the money amount in shops. however REPLACING it with c3 seemingly hides the money symbol without breaking anything
	ld c, $c3 ; deletes money symbol before owned money on trainer card but does not break anything else or affect any other instances money is shown
	call PrintBCDNumber
; Load the tile ID number for yen (=f0, viewable via VRAM viewer in GBG emulator, also written in charmap.asm) to print it directly after the printed BCD number
	ld a, $f0
; put yen sign after money sums - Write the tile number (= print the letter onscreen) to the tile map position. this part alone doesn't cause numbers to run on or break the game
	ld [hl], a
; end code to add yen symbol to after money amount in trainer card
	hlcoord 9, 6
	ld de, wPlayTimeHours ; hours
	lb bc, LEFT_ALIGN | 1, 3
	call PrintNumber
;  Load $d6 (the colon tile ID) into the memory location pointed to by HL (to separate hours and minutes)
	ld [hl], $d6
; Increment HL to move to the next on-screen position to display the next bit of text
	inc hl
	ld de, wPlayTimeMinutes ; minutes
; Load BC register pair with flags for leading zeroes and a width of 2 digits for the minutes
...
;	db $76,"BADGES",$76,"@"
	db $76,$70,$71,$72,$73,$74,$75,$76,"@" ; restores japanese "pokemon bajji" text on trainer card from leader_names.png
