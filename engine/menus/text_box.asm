...
DisplayMoneyBox:
	...
	hlcoord 12, 1
	ld de, wPlayerMoney
	ld c, $83; replacing with $a3 moves money digits + yen sign to far right of money box. $b3 glitches it.
	call PrintBCDNumber
; After printing the BCD number (sum of player's owned money in shop money box), print the yen symbol
    ld a, $f0  ; Loads the tile ID number for yen into register a
...
CurrencyString:
	db "      ¥@" ; stops yen sign from printing in front of money sum twice?
	db "      @" ; stops yen sign from printing in front of money sum twice?
